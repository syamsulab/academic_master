...
class AcademicSession(models.Model):
    ...
    start_date = fields.Date(
        string="Start Date", required=False, default=lambda self: time.strftime("%Y-%m-%d"))
    duration = fields.Integer(string="Duration", required=False)
    seats = fields.Integer(string="Seats", required=False)
    active = fields.Boolean(string="Active", default=True)
    attendee_ids = fields.One2many(
        comodel_name="academic.attendee", inverse_name="session_id", string="Attendees", required=False)
    ...
    def _cek_instruktur(self):
        for session in self:
            ...
            if session.instructor_id and session.instructor_id.id in attendee_partner_ids:
                raise ValidationError(_('Instructor cannot be an attendee in the same session.'))
